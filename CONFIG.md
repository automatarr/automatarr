...
You may alter your configuration with `automatarr cfg --help`, or find the direct location with `automatarr env info`.  
...
loading the device, automatarr will look in both the `WVDs` and `PRDs` directories
for a matching file.
...
Override the default directories used across automatarr.  
...
```yaml
downloads: "D:/Downloads/automatarr"
temp: "D:/Temp/automatarr"
```

There are directories not listed that cannot be modified as they are crucial to the operation of automatarr.
...
Choose what software to use to download data throughout automatarr where needed.
...
not work in automatarr. The API format can be seen in the [API Vault Code](automatarr/vaults/API.py).
...
- type: MySQL
  ...
  database: vault            # database used for automatarr
  username: jane11
  password: Doe123
```

I recommend giving only a trustable user (or yourself) CREATE permission and then use automatarr to cache at least one CEK
...
- Never use the root account with automatarr (even if it's you).
- Do not give multiple users the same username and/or password.
- Only give users access to the database used for automatarr.
- You may give trusted users CREATE permission so automatarr can create tables if needed.
...
- type: SQLite
  ...
  path: "C:/Users/Jane11/Documents/automatarr/data/key_vault.db"
```
...
Use [pywidevine] Serve-compliant Remote CDMs in automatarr as if it was a local widevine device file.  
...
Configuration data for pywidevine's serve functionality run through automatarr.
This effectively allows you to run `automatarr serve` to start serving pywidevine Serve-compliant CDMs right from your
...
# automatarr's WVDs directory for whatever reason
# devices:
